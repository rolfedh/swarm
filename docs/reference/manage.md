...
Before you can create a Swarm manager, get a discovery token from an existing discovery backend, or create a discovery backend.

Then use the `<discovery>` to specify a token generated by a discovery backend. For example,

    token://0ac50ef75c9739f5bfeeaf00503d4e6e

Or, use `<discovery>` to specify the discovery backend nodes. For example:
...
Advertise the IP address and port number of the Swarm manager. For example, `--advertise 172.30.0.161:4000`. Other swarm managers MUST be able to reach this swarm manager at this address.
